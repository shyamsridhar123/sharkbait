...
|-------|---------|-------------|
| `agent_start` | Ignored | "🦈 Coder agent starting..." |
| `handoff` | Ignored | "Delegating to reviewer..." |
| `replan` | Ignored | "⚠️ Re-planning: reason" |
...
```tsx
case "agent_start":
  setCurrentOutput(`🦈 ${event.agent} agent starting${event.mode ? ` (${event.mode} mode)` : ""}...\n`);
  break;
case "handoff":
  setCurrentOutput(prev => prev + `\n📤 Delegating to ${event.to}...\n`);
  break;
case "replan":
  setCurrentOutput(prev => prev + `\n⚠️ Re-planning: ${event.reason}\n`);
  break;
```
